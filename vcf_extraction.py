...
def pipeline_xml(filename_xml):
    ...
    results.to_csv(hgvsg_folder_path + filename_xml + '_hgvsg' + '.csv', index=False, sep='\t')
...
def pipeline(filename_vcf_gz):
    ...
    filename_vcf_gz_path = project_dir + '/' + input_dir + filename_vcf_gz
    ...
    for chunk in vcf:
        chunk.to_csv(temp_folder + 'vcf_sample.csv', sep=',', index=False, header=names)

    vcf_current = pd.read_csv(temp_folder + 'vcf_sample.csv', sep=',')
    ...
    final_vcf.to_csv(hgvsg_folder_path + 'outputHGVSg.csv', index=False, sep='\t')
...
if __name__ == '__main__':
    filename_vcf_gz_array = os.listdir(constants.input_dir_path)
