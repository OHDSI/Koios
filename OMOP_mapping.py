...
def map_to_omop(parsed_df,current_filename, vcf_mode = False):
    ...
    matched_to_synonyms = matched_to_synonyms.astype({"target_concept_id": int})
    ...
    matched_to_synonyms.to_csv(constants.output_dir_path + 'outputOMOP' + '.csv', index=False)

    print(constants.split_line_thin)
    num_matches = len(matched_to_synonyms_inner)
    print('The # of matches found: ', num_matches)
    if num_matches != 0:
        print(matched_to_synonyms_inner)
